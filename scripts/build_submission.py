...
`π-thon/` folder, zips it, and then re-verifies the archive from the outside:
...
def main() -> int:
    ...
    info = z.getinfo(f"{TEAM}/{TEAM}.csv")
    if not info.flag_bits & 0x800:
        problems.append("filename UTF-8 flag not set — the π may not survive extraction")
